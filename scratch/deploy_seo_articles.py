...
"""
        with open("remote_create_seo_posts.py", "w", encoding="utf-8") as f:
            f.write(script_code)
            
        print("Uploading creation script...")
        sftp.put("remote_create_seo_posts.py", "/app/remote_create_seo_posts.py")
        
        print("Executing script on server...")
        stdin, stdout, stderr = ssh.exec_command("cd /app && docker compose exec -T web python remote_create_seo_posts.py")
